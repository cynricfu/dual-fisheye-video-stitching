...
def buildMap(Ws, Hs, Wd, Hd, hfovd=160.0, vfovd=160.0):
    ...
    map_x = np.zeros((Wd, Hd), np.float32)
    map_y = np.zeros((Wd, Hd), np.float32)
    ...
    for x in range(0, int(Hd)):
        for y in range(0, int(Wd)):
            count = count + 1
            phi = vstart + (vfov * (float(x) / float(Hd)))
            theta = hstart + (hfov * (float(y) / float(Wd)))
...
def unwarp(img, xmap, ymap):
    ...
    result = Image(output[:, :, [2, 1, 0]], cv2image=True)
    return result


def postCrop(img, threshold=10):
    # Crop the image after dewarping
    return img.crop(img.width * 0.2, img.height * 0.1, img.width * 0.6, img.height * 0.8)
...
def findHomography(img, template, quality=500.00, minDist=0.2, minMatch=0.4):
    ...
    if(pr > minMatch and len(result) > 4):  # if more than minMatch % matches we go ahead and get the data
        ...
        homography, mask = cv2.findHomography(
            lhs_pt, rhs_pt, cv2.RANSAC, ransacReprojThreshold=1.1)
...
def constructMask(w, h, offset, expf=1.2):
    ...
    mask.drawRectangle(offset, 0, w - offset, h,
...
def main(argv):
    ...
    temp = img.crop((320, 0, 1280, img.height))
    sections = []
    ...
    Ws = int(temp.width)
    Hs = int(temp.height)
    Wd = int(temp.width * (4.0 / 3.0))
    Hd = int(temp.height)
    print "BUILDING MAP..."
    mapx, mapy = buildMap(Ws, Hs, Wd, Hd, 180.0, 180.0)
    print "MAP DONE"
    defished = []
